...
/*   Updated: 2018/11/15 15:13:13 by smondesi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

int		return_number(int ret, char *tmp, char **line)
{
	if (ret < 0)
		return (-1);
	if (ret == 0 && (tmp[0] == '\0'))
...
int		read_that(const int fd, char *tmp, char **mem, char **line)
{
	...
	return (return_number(var[0], tmp, line));
}
...
int		get_next_line(const int fd, char **line)
{
	...
	if ((!(tmp[0] = ft_strnew(0))) || fd < 0 || fd > OPEN_MAX)
		return (-1);
	if (mem[fd] != NULL)
	{
		if (ft_strchr(mem[fd], '\n'))
		{
			...
			free (tmp[1]);
			return((return_number(1, tmp[0], line)));
		}
		else
		{
